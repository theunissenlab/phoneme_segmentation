...
class DataSequence(object):
    ...
    """
    def __init__(self, data, split_inds, data_times=None, tr_times=None, time_starts=None, time_ends=None):
    ...
    def from_grid(cls, grid_transcript, trfile):
        ...
        if isinstance(data_entries[0], str):
            ...
            data = data_entries
        time_starts = np.array(list(map(float, list(zip(*grid_transcript))[0])))
        time_ends = np.array(list(map(float, list(zip(*grid_transcript))[1])))
        word_avgtimes = (time_starts + time_ends)/2.0
        
        tr = trfile.avgtr
        trtimes = trfile.get_reltriggertimes()
        
        split_inds = [(time_starts<(t+tr)).sum() for t in trtimes][:-1]
        return cls(data, split_inds, word_avgtimes, trtimes+tr/2.0, time_starts, time_ends)
